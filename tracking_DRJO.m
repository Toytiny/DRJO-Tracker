...
function [results] = tracking_DRJO(params)
...
[rs, cs]     = ndgrid( rg,cg);
y_0            = exp(-0.5 * (((rs.^2 + cs.^2) / output_sigma^2)));
%y0           = fft2(y); %   FFT of y.
interp_sz = use_sz;
Vy=0;
Vx=0;
...
min_sv_factor = max(CurrentSvFactor/search_area_scale,sqrt((5*search_area_scale)^2 /params.max_image_sample_size));
max_sv_factor = min(CurrentSvFactor*search_area_scale,search_area_scale*sqrt(size(im,1)^2 /params.max_image_sample_size));
...
for frame = 1:numel(s_frames)
    ...
    if frame>1
            center=pos+[Vy Vx];
            pos_pre=pos;
            pixel_template = get_pixels(im, center, round(sz*CurrentSvFactor), sz);
    ...
    if  frame == 1
        ...
        mu=params.init_mu;
        reg_sz(1)=max(round(beg_sz(1)*sqrt(CurrentArcFactor)),1);
        reg_sz(2)=max(round(beg_sz(2)/sqrt(CurrentArcFactor)),1);
        reg_sz(1)=min(reg_sz(1),use_sz(1)-2);
        reg_sz(2)=min(reg_sz(2),use_sz(2)-2);
        [~,~,w]=init_regwindow(use_sz,reg_sz,params);
    end
